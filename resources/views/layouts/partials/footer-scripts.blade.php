                       ...
                       if ('{{ explode('.', \Request::route()->getName())[0] }}' == 'certificates') {
                        $table.bootstrapTable('insertRow', {
                            ...
                            row: {
                                ...
                                status: '<button class="btn ' + (e.certificate.status == 'DRAFT' ? 'btn-warning' : (e.certificate.status == 'PENDING' ? 
                                        'btn-info' : (e.certificate.status == 'SIGNED' ? 'btn-success' : 'btn-danger'))) + 
                                        '"style="font-size: 14px;padding:0px;">' + e.status + '</button>',
                                type: '<button class="btn btn-warning" style="font-size: 14px;padding:0px;">' + e.certificate.type + '</button>',
           ...
           $("#markAsReadNotifications").click(function(e) {
               $.ajax({
                   ...
                   success: function(response) {
                       $('.indicator-nt').text('{{ Auth::user()->unreadNotifications->count() }}');
                   }
               });
           });
